...
(function () {
  ...
  if (_config.autoFetchSvg && !_config.fetchSvgFrom && DOCUMENT && DOCUMENT.currentScript) {
    ...
    if (src.search(fromJsDirectory) > -1) {
      if (!PRODUCTION) {
        console.info("Font Awesome SVG Auto-fetching URL has been determined using document.currentScript. This features is not supported in any version of Internet Explorer. ".concat(manuallyConfigureMessage));
      }

      _config.fetchSvgFrom = src.replace(fromJsDirectory, '/svgs');
    }
  }

  if (_config.fetchSvgFrom) {
    var a = DOCUMENT.createElement('a');
    a.href = _config.fetchSvgFrom;
    _config.fetchSvgFrom = a.href;
  }

  if (_config.autoFetchSvg && !_config.fetchSvgFrom && !PRODUCTION) {
    console.error("Disabling Font Awesome auto-fetching of SVG icons (it was enabled) because we could not guess the correct URL to load them from. ".concat(manuallyConfigureMessage));
    _config.autoFetchSvg = false;
  }
  /* END.FEATURE.AF */
    ...
    return new picked(function (resolve, reject) {
      ...
      if (iconName && prefix && config.autoFetchSvg) {
        return fetchSvg(prefix, iconName, _objectSpread({}, customIconParams)).then(function (icon) {
          ...
          resolve(_objectSpread({}, val, fetched));
        }).catch(reject);
      }
      /* END.FEATURE.AF */
  ...
  function replaceForPosition(node, position) {
    ...
    return new picked(function (resolve, reject) {
      ...
      if (alreadyProcessedPseudoElement && !fontFamily) {
        ...
        if (!iconName) {
          iconName = String.fromCharCode(parseInt(hexValue, 16));
        }
        /* END.FEATURE.AF */
